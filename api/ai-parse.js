export default async function handler(request, response) {
  ...
  const prompt = {
    ...
    messages: [
      {
        role: 'system',
        text: 'Ты — финансовый парсер. Твоя задача — извлечь транзакции из текста (это могут быть СМС, выписки или список операций). Для каждой транзакции найди: дату (в формате YYYY-MM-DD), название (магазин/отправитель), сумму (положительная для доходов, отрицательная для расходов), категорию (Еда, Транспорт, Зарплата и т.д.). Ответ должен быть строго в формате JSON: массив объектов [{"date": "2024-03-14", "name": "...", "amount": -100, "type": "expense", "category": "..."}]. Если год не указан в тексте, используй текущий 2025 год. Если транзакций нет, верни пустой массив [].'
      },
      {
        role: 'user',
